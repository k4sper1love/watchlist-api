...
func Serve() error {
	...
	if useHTTPS == "true" {
		...
		certFile := fmt.Sprintf("/etc/letsencrypt/live/%s/fullchain.pem", os.Getenv("SERVER_USER"))
		keyFile := fmt.Sprintf("/etc/letsencrypt/live/%s/privkey.pem", os.Getenv("SERVER_USER"))
